...
export default function LeaderboardPage() {
  ...
  const [userName, setUserName] = useState("Adebayo Ogundimu")
    ...
    const fetchData = async () => {
      try {
        ...
        const [lbRes, rankRes] = await Promise.all([
          leaderboard.getAll(period),
          leaderboard.getMyRank()
        ])
        setLeaderboardData((lbRes.data || lbRes).leaderboard || [])
  ...
  const filtered = tab === "all"
    ? leaderboardData
    : leaderboardData.filter((e) => tab === "naira" ? e.currency === "NGN" : e.currency === "USD")
      ...
      <div className="p-4 lg:p-6 space-y-6">
        ...
        {myRank?.hasRank ? (
            ...
            <div className="relative flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ background: "rgba(20,184,166,0.12)", border: "1px solid rgba(94,234,212,0.22)" }}>
                  <span className="text-lg font-black text-[#5eead4]">#{myRank.rank}</span>
                </div>
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">+{myRank.profitPct?.toFixed(1)}% profit this period</p>
                </div>
              </div>
          ...
          <div className="glass-card p-4 relative overflow-hidden">
            <div
              className="absolute inset-0"
              style={{ background: "radial-gradient(ellipse at 60% 50%, rgba(13,148,136,0.1) 0%, transparent 70%)" }}
            />
            <div className="relative flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ background: "rgba(167,255,235,0.08)", border: "1px solid rgba(167,255,235,0.15)" }}>
                  <Trophy size={20} className="text-muted-foreground" />
                </div>
                <div>
                  <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wider">Not Ranked Yet</p>
                  <p className="text-base font-bold text-foreground">Pass a challenge to get on the board!</p>
                  <p className="text-xs text-muted-foreground">Complete your first challenge to appear on the leaderboard.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="glass-card p-12 text-center">
            ...
            <p className="font-semibold text-foreground">No traders on the leaderboard yet</p>
            <p className="text-sm text-muted-foreground mt-1">Be the first to pass a challenge and top the rankings!</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="glass-card p-4 text-center order-1 self-end">
                ...
                <p className="text-lg font-black text-[#5eead4] mt-1">{top3[1]?.profitPct}%</p>
              ...
              >
                ...
                <div className="relative">
                  ...
                  <p className="text-2xl font-black text-[#ffd166] mt-1">{top3[0]?.profitPct}%</p>
                  <p className="text-xs text-muted-foreground">profit</p>
              ...
              <div className="glass-card p-4 text-center order-3 self-end">
                ...
                <p className="text-lg font-black text-[#5eead4] mt-1">{top3[2]?.profitPct}%</p>
                <p className="text-xs text-muted-foreground">profit</p>
              </div>
            </div>
              ...
              <div className="lg:col-span-3 glass-table overflow-hidden">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filtered.map((entry) => {
                        const isCurrentUser = myRank?.hasRank && entry.rank === myRank.rank
                        return (
                          <tr
                            key={entry.rank}
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <span className="font-bold text-[#4ade80]">+{entry.profitPct}%</span>
                            </td>
                            <td className="px-4 py-3">
                              <span
                                className={cn(
                            ...
                            <td className="px-4 py-3">
                              ...
                              {entry.profitPct > 5 && entry.profitPct <= 15 && (
