...
export default function TraderPerformance() {
  ...
  const [equityCurve, setEquityCurve] = useState<{d: string; val: number}[]>([]);
  ...
  const fetchTraders = async () => {
    try {
      setLoading(true);
      const response = await api.get("admin/trader-performance");
      const data = response.data;
      
      const tradersList: Trader[] = (data?.traders || []).map((t: any) => ({
        id: t.id || "",
        name: t.name || "",
        phase: t.phase || "",
        balance: t.balance || "₦0",
      ...
      if (tradersList.length > 0 && !selected) {
        setSelected(tradersList[0]);
      }
    } catch (err) {
  ...
  // Provide default values for safe rendering
  const safeSelected = selected || { id: "-", name: "No Trader Selected", phase: "-", balance: "₦0", profit: "₦0", pct: "0%", drawdown: "0%", trades: 0, winRate: "0%", status: "active", trend: "up" as const };
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="glass-card rounded-xl p-5">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="text-lg font-bold font-display text-white">{safeSelected.name}</h2>
                <p className="text-xs text-[#b9cbc2]/50">ID: #{safeSelected.id} · {safeSelected.phase}</p>
              </div>
              <span className={`text-[10px] font-semibold px-2 py-1 rounded-full uppercase ${statusStyles[safeSelected.status]}`}>
                {safeSelected.status}
              </span>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: "Balance", value: safeSelected.balance, color: "text-white" },
                { label: "Net P&L", value: safeSelected.profit, color: safeSelected.trend === "up" ? "text-[#00ffcc]" : "text-[#ff6b6b]" },
                { label: "Max Drawdown", value: safeSelected.drawdown, color: parseFloat(safeSelected.drawdown) > 8 ? "text-[#ff6b6b]" : "text-[#ffbc7c]" },
                { label: "Win Rate", value: safeSelected.winRate, color: "text-[#00ffcc]" },
              ].map(s => (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="glass-card rounded-xl p-4">
              ...
              <p className="text-2xl font-bold font-display text-white">{safeSelected.trades}</p>
            </div>
            <div className="glass-card rounded-xl p-4">
              ...
              <div className="flex items-center gap-2 mt-1">
                <div className="flex-1 h-2 bg-[#0b2f2d] rounded-full overflow-hidden">
                  <div className="h-full bg-[#00ffcc] rounded-full" style={{ width: safeSelected.phase === "Funded" ? "100%" : safeSelected.phase === "Phase 2" ? "66%" : "33%" }} />
                </div>
                <span className="text-xs text-[#00ffcc] font-semibold">{safeSelected.phase}</span>
              </div>
            </div>
          </div>
        </div>
