...
# ---------------------------------------------------------
# NEW FAQ HTML WITH PROPER JAVASCRIPT TOGGLES
# ---------------------------------------------------------
...
for i, (tab_name, faqs) in enumerate(tabs.items()):
    ...
    for j, faq in enumerate(faqs):
        ...
        # Using Javascript onclick toggles
        new_faq_html += f"""
...
# Add scripts (properly separated so python does not break tags)
...
function switchTab(tabId, btnElement) {{
    ...
    document.querySelectorAll('.faq-tab-btn-orig').forEach(btn => {{
        btn.className = "faq-tab-btn-orig inline-flex items-center cursor-pointer justify-center gap-2 whitespace-nowrap text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive border bg-transparent shadow-xs hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 h-11 rounded-3xl px-10 max-w-xs w-full";
    }});
    
    if(btnElement) {{
        btnElement.className = "faq-tab-btn-orig inline-flex items-center cursor-pointer justify-center gap-2 whitespace-nowrap text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive bg-[#A7FFEB] text-black hover:bg-[#A7FFEB]/90 h-11 rounded-3xl px-10 max-w-xs w-full";
    }}
}}
...
files_to_update = glob.glob('faqs*.html')

for f in files_to_update:
    with open(f, 'r', encoding='utf-8') as file:
        data = file.read()
        
    start_idx = data.find('<div class="flex flex-col sm:gap-5 gap-3 container mx-auto">')
    if start_idx != -1:
        
        # 1. Inject the anti-flash style tag into the head to permanently hide React's old FAQ container
        head_end = data.find('</head>')
        if head_end != -1 and 'id="anti-flash-style"' not in data:
            anti_flash_css = f"""
            <style id="anti-flash-style">
                /* Hide the old React FAQ container entirely by targeting its specific structure if it's not our new injected one */
                div.flex.flex-col.sm\\:gap-5.gap-3.container.mx-auto:not(#new-faqs-container) {{
            ...
            """
            data = data[:head_end] + anti_flash_css + data[head_end:]
            # Re-find start_idx due to string shift
            start_idx = data.find('<div class="flex flex-col sm:gap-5 gap-3 container mx-auto">')

        # 2. Add our new DOM node NEXT to the old hydrating component, not replacing it!
        # This stops Hydration Mismatches from removing our node.
        # So we insert it RIGHT BEFORE the old container.
        
        if '<div class="flex flex-col sm:gap-5 gap-3 container mx-auto" id="new-faqs-container"' not in data:
            new_data = data[:start_idx] + new_faq_html + data[start_idx:]
