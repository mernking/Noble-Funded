...
# Reset completely so we have a clean slate without double injected scripts
try:
...
for f in glob.glob('faqs*.html'):
    ...
    if '<div class="flex flex-col sm:gap-5 gap-3 container mx-auto">' in data:
        # 1. Inject Style to HEAD to prevent the original FAQ from jumping/flashing
        ...
        # 2. Add the robust Javascript at the absolute very end of <body> safely
        # We JSON encode the new HTML so there are NO syntax errors or string breaks in Javascript!
        ...
        document.addEventListener("DOMContentLoaded", () => {{
            ...
            // Try immediately
            if (!replaceFaq()) {{
                // Observe for hydration to finish replacing the DOM, then instantly swap it
        ...
        # safely insert near </body> without slicing off the footer!
