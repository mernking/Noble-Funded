    ...
    for j, faq in enumerate(faqs):
        ...
        <div class="border p-3 border-[#A7FFEB]/20 rounded-2xl last:border-b-0 bg-[#002B36] mt-4 overflow-hidden">
            <h3 class="flex font-normal m-0 relative">
                <button type="button" onclick="window.toggleAccordion('{idx}')" class="focus-visible:border-ring text-[#A7FFEB] focus-visible:ring-ring/50 flex flex-1 items-center justify-between py-4 text-sm font-medium transition-all outline-none md:text-xl sm:text-lg cursor-pointer w-full text-left bg-transparent border-none">
                    {q}
...
# CRITICAL FIX: Ensure new_faq_html has ABSOLUTELY NO script tags. It does not.
...
for f in glob.glob('faqs*.html'):
    ...
    if '<div class="flex flex-col sm:gap-5 gap-3 container mx-auto">' in data:
        # Instead of putting style in body where Next.js won't delete it, let's put it OUTSIDE the React DOM root 
        # But React hydrated the whole <body> anyway.
        # Actually, adding a <style> block directly before </body> works perfectly because React doesn't remove unknown nodes below its root unless it traverses them.
        
        # safely escape
        escaped_html = json.dumps(new_faq_html)
        # Prevents any `</script>` from breaking the JSON out of its boundary string
        escaped_html = escaped_html.replace('</script>', '<\\/script>')
        ...
        // Use a self-executing interval to ensure it replaces immediately without waiting for DOMContentLoaded (which might have already fired)
        (function() {{
            const replaceFaq = () => {{
                const oldFaq = document.querySelector('div.flex.flex-col.sm\\\\:gap-5.gap-3.container.mx-auto:not(#new-faqs-container)');
                if (oldFaq && oldFaq.innerHTML.includes('What type of Accounts do you offer')) {{
                    oldFaq.outerHTML = {escaped_html};
                    return true;
                }}
                return false;
            }};
            ...
            if (!replaceFaq()) {{
                ...
                setTimeout(() => clearInterval(interval), 5000); // safety fallback
        ...
        body_end = data.rfind('</body>')
        if body_end != -1:
            # We inject our style block and our script block RIGHT before body closes.
            # This is globally safe.
            data = data[:body_end] + js_injection + data[body_end:]
            
        with open(f, 'w', encoding='utf-8') as outfile:
            outfile.write(data)
        print(f"Perfect Javascript Injection in: {f}")
