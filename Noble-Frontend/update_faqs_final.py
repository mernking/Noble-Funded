...
current_tab = None; current_q = None; current_a = []
for line in md_content.split('\n'):
    if 'TAB 1: 🇳🇬 GETTING STARTED' in line: current_tab = 'Getting Started'
        ...
        if current_q and current_tab: tabs[current_tab].append({'question': current_q, 'answer': '\\n'.join(current_a).strip()})
        current_q = line.replace('### Q: ', '').strip(); current_a = []
    elif current_q and not line.startswith('---') and not line.startswith('## TAB'):
        if line.strip() != '': current_a.append(line)
if current_q and current_tab: tabs[current_tab].append({'question': current_q, 'answer': '\\n'.join(current_a).strip()})
    ...
    if '|---|' in a:
        ...
        for l in lines:
            if l.startswith('|') and '|---|' not in l:
                ...
                for c in cells: html_lines.append(f'<td class="p-3">{c}</td>')
...
new_faq_html = '<div class="flex flex-col sm:gap-5 gap-3 container mx-auto" id="new-faqs-container">'
...
for i, tab_name in enumerate(tabs.keys()):
    ...
    new_faq_html += f'<button onclick="window.switchTab(\'{tab_ids[i]}\', this)" class="faq-tab-btn-orig {t_class}">{tab_name}</button>'
new_faq_html += '</div><div data-slot="accordion" class="sm:mt-5 mt-0 max-w-7xl mx-auto w-full max-sm:px-5" data-orientation="vertical">'
for i, (tab_name, faqs) in enumerate(tabs.items()):
    ...
    for j, faq in enumerate(faqs):
        idx = f'{i}-{j}'; q = faq['question']; a_html = md_to_html(faq['answer'])
        new_faq_html += f"""<div class="border p-3 border-[#A7FFEB]/20 rounded-2xl last:border-b-0 bg-[#002B36] mt-4 overflow-hidden"><h3 class="flex font-normal m-0 relative"><button type="button" onclick="window.toggleAccordion('{idx}')" class="focus-visible:border-ring text-[#A7FFEB] focus-visible:ring-ring/50 flex flex-1 items-center justify-between py-4 text-sm font-medium transition-all outline-none md:text-xl sm:text-lg cursor-pointer w-full text-left bg-transparent border-none">{q}<svg id="icon-faq-{idx}" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-down text-muted-foreground shrink-0 transition-transform duration-200 text-white w-8 h-8 font-light" style="transform: rotate(0deg);"><path d="m6 9 6 6 6-6"></path></svg></button></h3><div id="content-faq-{idx}" class="overflow-hidden transition-all duration-300 ease-in-out" style="max-height: 0px;"><div class="pb-4 pt-0"><div class="text-xs sm:text-sm font-light leading-10">{a_html}</div></div></div></div>"""
    new_faq_html += '</div>'
new_faq_html += '</div></div>'


try: subprocess.run(["git", "checkout", "faqs.html"], check=True)
except: pass

with open('faqs.html', 'r', encoding='utf-8') as file:
    data = file.read()

orig_start = '<div class="flex flex-col sm:gap-5 gap-3 container mx-auto">'
if orig_start in data:
    html_replaced = data.replace(orig_start, '<div id="hide-me-faqs" class="flex flex-col sm:gap-5 gap-3 container mx-auto">', 1)
    
    escaped_html = json.dumps(new_faq_html).replace('</script>', '<\\/script>')
    
    js_injection = f"""
    <style>
        #hide-me-faqs {{ display: none !important; visibility: hidden !important; }}
        div.flex.flex-col.sm\\:gap-5.gap-3.container.mx-auto:not(#new-faqs-container) {{ display: none !important; }}
    </style>
    <script>
    window.switchTab = function(tabId, btnElement) {{
        document.querySelectorAll('.faq-tab-content-orig').forEach(el => el.style.display = 'none');
        document.getElementById(tabId).style.display = 'block';
        document.querySelectorAll('.faq-tab-btn-orig').forEach(btn => {{ btn.className = "{inactive_tab_classes}"; }});
        if(btnElement) {{ btnElement.className = "{active_tab_classes}"; }}
    }};
    window.toggleAccordion = function(idx) {{
        ...
        if (content.style.maxHeight === '0px' || content.style.maxHeight === '') {{
            content.style.maxHeight = '3000px'; icon.style.transform = 'rotate(180deg)';
        }} else {{
    ...
    (function() {{
        const targetString = 'What type of Accounts do you offer';
        const replaceFaq = () => {{
            let allDivs = document.querySelectorAll('div');
            for(let i=0; i<allDivs.length; i++) {{
                let d = allDivs[i];
                if(d.innerHTML && d.innerHTML.includes(targetString) && d.innerHTML.includes('Pay-outs</button>') && !d.id.includes('new-faqs')) {{
                    // Ensure we target the outermost container by checking if its PARENT doesn't contain it multiple times
                    if(d.parentElement && d.parentElement.innerHTML.split(targetString).length === 2 && d.className && d.className.includes('container mx-auto')) {{
                        d.outerHTML = {escaped_html};
                        return true;
                    }}
                }}
            }}
        ...
        const interval = setInterval(() => {{
            if (replaceFaq()) clearInterval(interval);
            if (++attempts > 200) clearInterval(interval);
        }}, 50);
        
        const observer = new MutationObserver(() => {{
            if (replaceFaq()) observer.disconnect();
        }});
    ...
    body_end = html_replaced.rfind('</body>')
    if body_end != -1:
        html_replaced = html_replaced[:body_end] + js_injection + html_replaced[body_end:]
        
    with open('faqs.html', 'w', encoding='utf-8') as outfile:
        outfile.write(html_replaced)
    print("Injected strictly.")
