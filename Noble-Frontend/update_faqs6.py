...
for f in glob.glob('faqs*.html'):
    ...
    if orig_start in data:
        # Give the React container an ID to guarantee it hides on first paint
        ...
        (function() {{
            const replaceFaq = () => {{
                let targetFaq = document.getElementById('hide-me-faqs');
                if (!targetFaq) {{
                    // If hydration stripped the ID, fallback to DOM scanning
                    const divs = document.querySelectorAll('div');
                    for (let i = 0; i < divs.length; i++) {{
                        if (divs[i].innerHTML.includes('Pay-outs</button>') && !divs[i].id.includes('new-faqs-container')) {{
                            targetFaq = divs[i];
                            break;
                        }}
                    }}
                }}
                if (targetFaq && targetFaq.innerHTML.includes('What type of Accounts do you offer')) {{
                    targetFaq.outerHTML = {escaped_html};
                    return true;
                }}
                return false;
            }};
        ...
        print(f"Foolproof DOM Scanner Injected in: {f}")
